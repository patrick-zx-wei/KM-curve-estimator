...
from km_estimator.utils.gemini_client import validate_image, validate_image_async
...
async def input_guard_async(state: PipelineState) -> PipelineState:
    """
    Async: Validate that the image is a proper Kaplan-Meier survival curve.
    ...
    # Retry only on API errors, not on invalid images
    for attempt in range(1, cfg.max_input_guard_retries + 1):
        result = await validate_image_async(
            image_path,
            timeout=cfg.api_timeout_seconds,
            max_retries=cfg.api_max_retries,
        )
