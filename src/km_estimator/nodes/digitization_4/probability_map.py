    ...
    for arm_name in arm_names:
        ...
        if model.reliability >= COLOR_HARD_LOCK_RELIABILITY:
            ...
            if (
                ...
                and hard_cov >= COLOR_HARD_LOCK_MIN_COLUMN_COVERAGE
            ):
                arm_mask = hard_mask
                base = np.where(arm_mask, base, base - 1.80).astype(np.float32)
                warnings.append(f"I_COLOR_HARD_LOCK:{arm_name}:{hard_density:.4f}:{hard_cov:.3f}:{hard_thr:.3f}")
            else:
                warnings.append(f"W_COLOR_HARD_LOCK_SPARSE:{arm_name}:{hard_density:.4f}:{hard_cov:.3f}:{hard_thr:.3f}")
