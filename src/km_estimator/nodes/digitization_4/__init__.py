...
def _sample_hardpoint_color_hints(
    image: np.ndarray,
    plot_model,
    hardpoint_guides: dict[str, tuple[tuple[int, int], ...]],
) -> tuple[dict[str, tuple[float, float, float]], list[str]]:
    warnings: list[str] = []
    if not hardpoint_guides:
        return {}, warnings
    ...
    value = roi_hsv[:, :, 2].astype(np.float32)
    invalid = (plot_model.axis_mask > 0) | (plot_model.tick_mask > 0)

    hints: dict[str, tuple[float, float, float]] = {}
    h, w = roi_lab.shape[:2]
    radius = max(1, int(round(min(h, w) * 0.0045)))
    min_samples = 10
    sat_min = 48.0
    val_max = 245.0

    for arm_name, anchors in hardpoint_guides.items():
        if len(anchors) < 2:
            continue
        samples: list[np.ndarray] = []
        for cx, cy in anchors:
            x_lo = max(0, int(cx) - radius)
            x_hi = min(w, int(cx) + radius + 1)
            y_lo = max(0, int(cy) - radius)
            y_hi = min(h, int(cy) + radius + 1)
            if x_lo >= x_hi or y_lo >= y_hi:
                continue
            patch_lab = roi_lab[y_lo:y_hi, x_lo:x_hi, :]
            patch_sat = sat[y_lo:y_hi, x_lo:x_hi]
            patch_val = value[y_lo:y_hi, x_lo:x_hi]
            patch_invalid = invalid[y_lo:y_hi, x_lo:x_hi]
            good = (~patch_invalid) & (patch_sat >= sat_min) & (patch_val <= val_max)
            if not np.any(good):
                continue
            patch_samples = patch_lab[good]
            if patch_samples.size <= 0:
                continue
            samples.append(np.asarray(patch_samples, dtype=np.float32))
        if not samples:
            warnings.append(f"W_HARDPOINT_COLOR_HINT_SPARSE:{arm_name}:0")
            continue
        stacked = np.concatenate(samples, axis=0).reshape(-1, 3)
        if stacked.shape[0] < min_samples:
            warnings.append(f"W_HARDPOINT_COLOR_HINT_SPARSE:{arm_name}:{stacked.shape[0]}")
            continue
        median = np.median(stacked, axis=0).astype(np.float32)
        hint = (float(median[0]), float(median[1]), float(median[2]))
        hints[arm_name] = hint
        warnings.append(f"I_HARDPOINT_COLOR_HINT:{arm_name}:{stacked.shape[0]}")
...
def digitize_v4(state: PipelineState) -> PipelineState:
    ...
    hardpoint_color_hints, hardpoint_color_warnings = _sample_hardpoint_color_hints(
        image=image,
        plot_model=plot_model,
        hardpoint_guides=hardpoint_guides,
    )
    all_warnings.extend(hardpoint_color_warnings)

    color_models, color_warnings = build_color_models(image, state.plot_metadata, plot_model)
    all_warnings.extend(color_warnings)

    for arm_name, hint_lab in hardpoint_color_hints.items():
        model = color_models.get(arm_name)
        if model is None:
            continue
        prior_dist = _lab_distance(model.expected_lab, hint_lab)
        if prior_dist is not None:
            all_warnings.append(f"I_HARDPOINT_COLOR_PRIOR_DIST:{arm_name}:{prior_dist:.1f}")
        color_models[arm_name] = model.__class__(
            name=model.name,
            expected_lab=model.expected_lab,
            observed_lab=hint_lab,
            source="hardpoint_hint",
            valid=True,
            reliability=max(0.92, float(model.reliability)),
